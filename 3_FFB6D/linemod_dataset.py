...
class Dataset():
    ...
    def __init__(self, dataset_name, cls_type="duck", DEBUG=False):
        ...
        """
        self.root = os.path.join(self.config.lm_root, 'Linemod_preprocessed')  # path to the folder - root
        self.cls_root = os.path.join(self.root, "data/%02d/" % self.cls_id)  # path to the corresponding in the root
        """
        numpy.random
        """
        self.rng = np.random
        """
        loading meta file from the LineMod dataset 
            This can be changed to 'gt.json' for BlenderProc data. 
            The annotations of the RasterTriangle data is not enumerated in this file, these annotation are found in the 
            individual .pkl files.
        """
        meta_file = open(os.path.join(self.cls_root, 'gt.yml'), "r")
        self.meta_lst = yaml.load(meta_file)
        """
        loading the directories to the training images  
            Which data needs to be loaded can changed here. 
            When no RasterTriangle data is used, change the following: 
                self.rnd_lst = []
                self.fuse_lst = []
        """
        if dataset_name == 'train':
            ...
            # loading the "real" data from the LineMod dataset
            ...
            # loading the Rendered data from RasterTriangle
            ...
            # loading the Fused data from RasterTriangle
        ...
        """
        loading the directories to the test images
        """
        else:
    ...
    def real_gen(self):
        ...
        Get an image form the 'real list'
            This 'real list' refers to the LineMod dataset.
        """
        n = len(self.real_lst)
    ...
    def add_real_back(self, rgb, labels, dpt, dpt_msk):
        real_item = self.real_gen()  # image from the "real" list, LineMod dataset
        """
        data fusion process 
    ...
    def get_item(self, item_name):
        if "pkl" in item_name:
            ...
            """
            loading an image of the LineMod dataset
                When loading BlenderProc data, this is done with the same code. 
            """
