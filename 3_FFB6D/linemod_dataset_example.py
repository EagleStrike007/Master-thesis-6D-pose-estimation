...
class Dataset():
    ...
    def __init__(self, dataset_name, cls_type="duck", DEBUG=False):
        ...
        """
        Root to the folder of the dataset. 
        """
        self.root = os.path.join(self.config.lm_root, 'Linemod_preprocessed')
        """
        The training data is loaded form 4 different folders. 
        Therefore, 4 different train.txt files are made. 
        Depending in which folder the train.txt is located, the name of the image is preceded by the number of the folder. 
        For example in folder '2', image '000010': "2/000010"
        
        The annotations are stored in a .json file. The structure of this file is similar to the .yml file from the 
        LineMod dataset. 
        """
        if dataset_name == "train":
            # loading directory to images out folder 1
            # loading the annotation form a .json file
            self.cls_root_1 = os.path.join(self.root, "data/%02d-train-1/" % self.cls_id)
            meta_file = open(os.path.join(self.cls_root_1, 'gt.json'), "r")
            self.meta_lst_1 = yaml.load(meta_file)
            print("loading meta", self.cls_root_1)

            # loading directory to images out folder 2
            # loading the annotation form a .json file
            self.cls_root_2 = os.path.join(self.root, "data/%02d-train-2/" % self.cls_id)
            meta_file = open(os.path.join(self.cls_root_2, 'gt.json'), "r")
            self.meta_lst_2 = yaml.load(meta_file)
            print("loading meta", self.cls_root_2)

            # loading directory to images out folder 3
            # loading the annotation form a .json file
            self.cls_root_3 = os.path.join(self.root, "data/%02d-train-3/" % self.cls_id)
            meta_file = open(os.path.join(self.cls_root_3, 'gt.json'), "r")
            self.meta_lst_3 = yaml.load(meta_file)
            print("loading meta", self.cls_root_3)

            # loading directory to images out folder 4
            # loading the annotation form a .json file
            self.cls_root_4 = os.path.join(self.root, "data/%02d-train-4/" % self.cls_id)
            meta_file = open(os.path.join(self.cls_root_4, 'gt.json'), "r")
            self.meta_lst_4 = yaml.load(meta_file)
            print("loading meta", self.cls_root_4)
        """
        loading the directory to the test/validation dataset 
        """
        else:
            self.cls_root = os.path.join(self.root, "data/%02d-val/" % self.cls_id)
            meta_file = open(os.path.join(self.cls_root, 'gt.json'), "r")
            self.meta_lst = yaml.load(meta_file)
            print("loading meta", self.cls_root)
        self.rng = np.random
        if dataset_name == 'train':
            """
            The training data is found in 4 different folders. Therefore, 4 different lists are made. 
            These 4 lists are then combined to one big list. 
            
            Note: RasterTriangle data can still be loaded. 
            """
            self.add_noise = True
            real_img_pth = os.path.join(
                self.cls_root_1, "train.txt"
            )
            self.real_lst_1 = self.bs_utils.read_lines(real_img_pth)

            real_img_pth = os.path.join(
                self.cls_root_2, "train.txt"
            )
            self.real_lst_2 = self.bs_utils.read_lines(real_img_pth)

            real_img_pth = os.path.join(
                self.cls_root_3, "train.txt"
            )

            self.real_lst_3 = self.bs_utils.read_lines(real_img_pth)

            real_img_pth = os.path.join(
                self.cls_root_4, "train.txt"
            )
            self.real_lst_4 = self.bs_utils.read_lines(real_img_pth)

            self.real_lst = self.real_lst_1 + self.real_lst_2 +self.real_lst_3 + self.real_lst_4
        ...
        """
        loading the test/validation dataset 
        """
        else:
    ...
    def real_gen(self):
        """
        Get an image form the 'real list'
            This 'real list' refers to the BlenderProc dataset.
        """
        n = len(self.real_lst)
    ...
    def linear_motion_blur(self, img, angle, length):
        """
        Adding motion blur onto RGB image
        """
    ...
    def rgb_add_noise(self, img):
        ...
        """
        data augmentation
            Noise is addd into the RGB image. 
        """
    ...
    def add_real_back(self, rgb, labels, dpt, dpt_msk):
        item_name = self.real_gen()  # loading data from the BlenderProc dataset
        """
        the different folders are split into the 4 different "root/folder"
            Depending on the number preceding the image number, the correct root/folder is selected. 
            For example: "2/000100", folder number '2' image '000100' 
        """
        if '1' in item_name[:2]:
            item_name = item_name[2:]
            with Image.open(os.path.join(self.cls_root_1, "depth", item_name+'.png')) as di:
                real_dpt = np.array(di)
            with Image.open(os.path.join(self.cls_root_1, "mask", item_name+'.png')) as li:
                bk_label = np.array(li)
            bk_label = (bk_label < 255).astype(rgb.dtype)
            if len(bk_label.shape) > 2:
                bk_label = bk_label[:, :, 0]
            with Image.open(os.path.join(self.cls_root_1, "rgb", item_name+'.png')) as ri:
                back = np.array(ri)[:, :, :3] * bk_label[:, :, None]

        elif '2' in item_name[:2]:
            item_name = item_name[2:]
            with Image.open(os.path.join(self.cls_root_2, "depth", item_name + '.png')) as di:
                real_dpt = np.array(di)
            with Image.open(os.path.join(self.cls_root_2, "mask", item_name + '.png')) as li:
                bk_label = np.array(li)
            bk_label = (bk_label < 255).astype(rgb.dtype)
            if len(bk_label.shape) > 2:
                bk_label = bk_label[:, :, 0]
            with Image.open(os.path.join(self.cls_root_2, "rgb", item_name + '.png')) as ri:
                back = np.array(ri)[:, :, :3] * bk_label[:, :, None]

        elif '3' in item_name[:2]:
            item_name = item_name[2:]
            with Image.open(os.path.join(self.cls_root_3, "depth", item_name + '.png')) as di:
                real_dpt = np.array(di)
            with Image.open(os.path.join(self.cls_root_3, "mask", item_name + '.png')) as li:
                bk_label = np.array(li)
            bk_label = (bk_label < 255).astype(rgb.dtype)
            if len(bk_label.shape) > 2:
                bk_label = bk_label[:, :, 0]
            with Image.open(os.path.join(self.cls_root_3, "rgb", item_name + '.png')) as ri:
                back = np.array(ri)[:, :, :3] * bk_label[:, :, None]

        elif '4' in item_name[:2]:
            item_name = item_name[2:]
            with Image.open(os.path.join(self.cls_root_4, "depth", item_name + '.png')) as di:
                real_dpt = np.array(di)
            with Image.open(os.path.join(self.cls_root_4, "mask", item_name + '.png')) as li:
                bk_label = np.array(li)
            bk_label = (bk_label < 255).astype(rgb.dtype)
            if len(bk_label.shape) > 2:
                bk_label = bk_label[:, :, 0]
            with Image.open(os.path.join(self.cls_root_4, "rgb", item_name + '.png')) as ri:
                back = np.array(ri)[:, :, :3] * bk_label[:, :, None]

        dpt_back = real_dpt.astype(np.float32) * bk_label.astype(np.float32)
        """
        data fusion process 
            Two images are merged into a new image.
            This is done with an image of the RasterTriangle and BlenderProc. 
        """
        if self.rng.rand() < 0.9:
            ...
            dpt = dpt_back * (msk_back[:, :, 0] != 0).astype(rgb.dtype) + dpt * (msk_back[:, :, 0] == 0).astype(rgb.dtype)
    ...
    def dpt_2_pcld(self, dpt, cam_scale, K):
        if len(dpt.shape) > 2:
            dpt = dpt[:, :, 0]
        """
        converting the depth image to a point cloud 
        """
    ...
    def get_item(self, item_name):
        if "pkl" in item_name:
            """
            loading an image of the RasterTriangle data 
            """
            ...
            """
            loading an image from the BlenderProc dataset 
            """
            if self.dataset_name == "train":
                ...
                """
                if '1' in item_name[:2]:
                    with Image.open(os.path.join(self.cls_root_1, "depth/{}.png".format(item_name[2:]))) as di:
                        dpt_mm = np.array(di)
                    with Image.open(os.path.join(self.cls_root_1, "mask/{}.png".format(item_name[2:]))) as li:
                        labels = np.array(li)
                        labels = (labels > 0).astype("uint8")
                    with Image.open(os.path.join(self.cls_root_1, "rgb/{}.png".format(item_name[2:]))) as ri:
                        if self.add_noise:
                            ri = self.trancolor(ri)
                        rgb = np.array(ri)[:, :, :3]
                    meta = self.meta_lst_1[str(int(item_name[2:]))]
                elif '2' in item_name[:2]:
                    with Image.open(os.path.join(self.cls_root_2, "depth/{}.png".format(item_name[2:]))) as di:
                        dpt_mm = np.array(di)
                    with Image.open(os.path.join(self.cls_root_2, "mask/{}.png".format(item_name[2:]))) as li:
                        labels = np.array(li)
                        labels = (labels > 0).astype("uint8")
                    with Image.open(os.path.join(self.cls_root_2, "rgb/{}.png".format(item_name[2:]))) as ri:
                        if self.add_noise:
                            ri = self.trancolor(ri)
                        rgb = np.array(ri)[:, :, :3]
                    meta = self.meta_lst_2[str(int(item_name[2:]))]
                elif '3' in item_name[:2]:
                    with Image.open(os.path.join(self.cls_root_3, "depth/{}.png".format(item_name[2:]))) as di:
                        dpt_mm = np.array(di)
                    with Image.open(os.path.join(self.cls_root_3, "mask/{}.png".format(item_name[2:]))) as li:
                        labels = np.array(li)
                        labels = (labels > 0).astype("uint8")
                    with Image.open(os.path.join(self.cls_root_3, "rgb/{}.png".format(item_name[2:]))) as ri:
                        if self.add_noise:
                            ri = self.trancolor(ri)
                        rgb = np.array(ri)[:, :, :3]
                    meta = self.meta_lst_3[str(int(item_name[2:]))]
                elif '4' in item_name[:2]:
                    with Image.open(os.path.join(self.cls_root_4, "depth/{}.png".format(item_name[2:]))) as di:
                        dpt_mm = np.array(di)
                    with Image.open(os.path.join(self.cls_root_4, "mask/{}.png".format(item_name[2:]))) as li:
                        labels = np.array(li)
                        labels = (labels > 0).astype("uint8")
                    with Image.open(os.path.join(self.cls_root_4, "rgb/{}.png".format(item_name[2:]))) as ri:
                        if self.add_noise:
                            ri = self.trancolor(ri)
                        rgb = np.array(ri)[:, :, :3]
                    meta = self.meta_lst_4[str(int(item_name[2:]))]
                ...
                with Image.open(os.path.join(self.cls_root, "rgb/{}.png".format(item_name))) as ri:
                    if self.add_noise:
                        ri = self.trancolor(ri)
                    rgb = np.array(ri)[:, :, :3]
                meta = self.meta_lst[str(int(item_name))]
            ...
            if self.cls_id == 2:
                ...
                meta = meta[0]
            """
            converting the annotations to a different unit 
                The translation is changed form mm to m. 
            """
        ...
        """
        adding noise
            In the training noise is added to training data. 
        """
        if self.add_noise:
            if self.rng.rand() < 0.8 or '2' in item_name[:2]:
                # noise is added into the RGB image
        ...
        """
        creation of the normal map 
        """
        dpt_mm = dpt_mm.copy().astype(np.uint16)
        nrm_map = normalSpeed.depth_normal(
            dpt_mm, K[0][0], K[1][1], 5, 10000, 20, False
        )
        if self.DEBUG:
