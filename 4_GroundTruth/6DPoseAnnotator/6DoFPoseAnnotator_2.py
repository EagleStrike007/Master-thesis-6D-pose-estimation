...
if __name__ == "__main__":
    ...
    max_counter = 1  # number of images that is processed
    begin = 0  # number of the first image of the dataset
    offset_im = 0  # number of image from where to start loading
    offset_trans = offset_im - begin
    ...
    for i in range(max_counter):
        ...
        print(":: Load RGB")
        rgb_path = "data/Pose_1/color/{:06d}.png".format(counter + offset_im)
        depth_path = "data/Pose_1/depth/{:06d}.png".format(counter + offset_im)
        ...
        When no .json is present an default setting is used!
        """
        ...
        if os.path.exists( init ):
            ...
            try:
                initial_trans = np.array(data[counter + offset_trans]["transformation4x4"])[0]
            except:
                initial_trans = np.array(data[0]["transformation4x4"])[0]
