"""
Code based one:
https://github.com/ethnhe/raster_triangle/tree/master
"""
...
"""
dictionary with object name linked to the object id 
"""
lm_obj_dict={
    'ape':1,
    'benchvise':2,
...
"""
root to the dataset
    This includes the 3D model and images. 
"""
root = './'
...
def collect_linemod_set_info(
    ...
    """
    create .pkl file from the different objects 
    """
    i = 0
    j = 0
    while i < 10000:
        try:  
            path = "./renders/{}/{}.pkl".format(linemod_cls_name, j)
            data_pkl = pkl.load(open(path, "rb"))
            data = {}
            data['rgb_pth'] = path #data_pkl['rgb']
            data['dpt_pth'] = path #data_pkl['depth']
            data['msk_pth'] = path #data_pkl['mask']
            RT = data_pkl['RT']
            data['RT'] = RT
...
def randomly_sample_foreground(image_db, linemod_dir):
    ...
    """
    read image from the .pkl file 
    """

    data_pkl = pkl.load(open(image_db[idx]['rgb_pth'], "rb"))
    di = data_pkl['depth'] * 1000  # convert depth image to meter
...
def save_fuse_data(
    ...
    data['K'] = Intrinsic_matrix['blender']
    data['RT'] = t_pose
    data['cls_typ'] = cls
    data['rnd_typ'] = 'fuse'
    data['begins'] = fuse_begins
    if DEBUG:
        cv2.imshow("rgb", fuse_img[:, :, ::-1])
        cv2.imshow("depth", (fuse_depth / fuse_depth.max() * 255).astype('uint8'))
        cv2.imshow("label", (fuse_mask / fuse_mask.max() * 255).astype("uint8"))
        cv2.waitKey(0)
